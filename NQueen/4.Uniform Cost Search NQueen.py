...
class NQueensGUI:
    def __init__(self, size):
        ...
        self.solve_button = tk.Button(self.window, text="Solve", command=self.solve_uniform_cost)
    ...
    def solve_uniform_cost(self):
        self.solutions = self.solve_nqueens_uniform_cost(self.size)
    ...
    def solve_nqueens_uniform_cost(self, size):
        if size < 1:
            return []

        solutions = []
        open_list = [{"queens": [], "cost": 0}]
        closed_list = set()

        while open_list:
            open_list.sort(key=lambda x: x["cost"])
            current = open_list.pop(0)
            queens, cost = current["queens"], current["cost"]
            ...
            for col in range(size):
                ...
                new_cost = cost + 1
                open_list.append({"queens": new_queens, "cost": new_cost})
