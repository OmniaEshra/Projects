        ...
        for row in range(self.game.board_size):
            for col in range(self.game.board_size):
                if self.game.solution and self.game.solution[row][col] == 1:
                    ...
                    self.canvas.create_oval(x - self.square_size * 0.3, y - self.square_size * 0.3,
                                            x + self.square_size * 0.3, y + self.square_size * 0.3, fill="lavender")
...
if __name__ == "__main__":
    board_size = 4 # Change the board size as desired
